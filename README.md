...
```yaml
convert-vanilla-exp: true
```

启用后，玩家获得的原版经验会被拦截，并转入 Roguelike 等级系统。
...
```text
plugins/Roguelike/mobs/default_mobs.json
```
...
默认关闭内置 scoreboard，推荐使用 TAB + PlaceholderAPI 展示 Roguelike 数据。

如果确实不用 TAB，可以手动开启内置 scoreboard：
...
管理员可以执行 `/rw init` 自动关闭内置 scoreboard，并生成 TAB/MythicMobs 示例配置文件。
...
```text
plugins/Roguelike/weapons/default_weapons.json
```
